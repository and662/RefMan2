...
I wrote it like this for 2 reasons: 1. refman is a work in progress so this way you can easily download a newer version of .refman without it messing with your saved references. And 2. you can rename or make multiple "References" folders and databases, or move the folder and refman2 will work fine (since the code is in a subdirectory). 
...
You may think it's a good idea to create different databases for each bibliography you need to format. However I plan to create "reference groups" which will be tags associated to each citekey. So you can distinguish a set of citkeys by assigning them to a "thesis" reference group for example.  
