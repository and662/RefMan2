...
└── .refman2 
#+end_example
Where the first 2 folders store PDFs and these directories are created automatically if they do not exist the first time you run refman2. =References/citekeys.db= stores all the citation metadata in an SQLite database, and =References/.refman= stores the refman code. Although all of this can be customized (and documentation for that is coming soon). This design allows you to make multiple "References" folders, and databases, and it allows you to keep your PDFs seperate from the refman code.
